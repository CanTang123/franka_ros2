...
关节曲线为订阅收到的实测 q/dq；如果有控制器 feedback，位置图另外用虚线画其 desired 目标。末端便利话题默认最多按 50 Hz 写入日志，可用 `--ee-record-rate` 在 1–200 Hz 内调整；每条消息的原 ROS 时间戳仍保留。这避免硬件默认 1 kHz 便利话题的 JSON 写盘挤占控制回调。关节状态仍按收到的消息频率记录；不保证网络丢包时仍有完整硬件频率。CSV 同时保留本机接收时间和源 ROS 时间戳，各传感器序列不强行插值对齐。图的时间原点为 enable，竖虚线表示 disable，后续数据展示停止过程。
...
- 默认 `--time-scale 1` 保持原始 0.05s 节点间隔。对于原始离散轨迹速度/加速度超过现场保守门限的演示，可显式使用 `--time-scale 3`：模型相位、总时长、0.2s 重规划周期、轨迹节点时间以及 ROS 碰撞校验预算统一放慢三倍，模型仍按 control_step=0/4/8/... 前进。HTTP 推理超时和状态新鲜度门限不放宽；速度上限、碰撞检查和 URDF 限位也不放宽。日志会记录缩放值。A/B/C/D 必须使用相同缩放值。
- 规划器返回 H16（模型时间 0.8s）绝对关节目标；实机一次只授权前 4 个目标。`time-scale=1` 时每个节点 0.05s、请求周期 0.2s；缩放模式下二者同比例放慢。随机源仍按 control_step=0/4/8/... 折叠。
- 每段 ROS 轨迹以校验过的实测起点为 t=0；`time-scale=1` 时四个目标放在 0.05/0.10/0.15/0.20s。已有 JointTrajectoryController 会插值位置并通过 effort PID 执行，**这与仿真零阶保持位置伺服不是相同底层协议**。需记录时间缩放、控制器版本、增益、现场工具和负载；这里的实机结果作为独立实验报告。
- `time-scale=1` 时整个网络/推理/MoveIt 校验预算为 150ms；缩放模式下，仅 ROS 侧总校验预算同比例增加，为更长轨迹段的碰撞采样留出时间，HTTP 推理超时仍为 150ms。状态最大延迟 100ms，关节起点容差 0.03rad，基础单目标步长 0.03rad，测得及目标段平均速度上限 0.6rad/s。超时、状态异常、目标跳变、限位、碰撞检查拒绝、控制器失败均停止并请求取消，不裁剪模型动作掩盖失败。
- MoveIt 在当前起点与四个关节线性目标之间每至多 0.01rad 采样检查；这是有限采样，不能当作连续碰撞证明，且依赖现场 Planning Scene 正确包含桌面、工具等对象。
- 若某个 `/check_state_validity` 异步请求在进入碰撞检查阶段 20ms 后仍未返回，客户端只重发尚未完成的采样点一次；请求发出后的总预算仍为 150ms，并未放宽。所有采样必须在总预算内明确返回 `valid=true`，否则停止且不发送该段轨迹。`collision_retry` 日志记录重试样本数、碰撞阶段耗时及剩余预算。
